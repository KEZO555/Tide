...
data class LightToolMetadata(
    ...
    /** Declared tool capabilities (e.g. "detached-audio"). Each capability
     *  contributes its own permissions, manifest entries, and a marker the SDK
     *  checks at runtime — see [LightToolPolicy]. */
...
object LightToolPolicy {
    ...
    /** The "detached-audio" capability: SDK-owned background media playback. */
     ...
     * deliberately absent from [ALLOWED_PERMISSIONS]: a capability is the single
     * source of truth for the permissions it needs, so a tool declares the
     * capability rather than the bare platform permissions.
     */
     ...
     * own namespace, so only this generator can produce it — the property a bare
     * platform permission cannot have. The SDK checks it at runtime.
     */
